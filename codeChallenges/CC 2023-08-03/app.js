...
// Return: the argument array with the largest integer pushed to the end of the array
...
const bubblesortOnce = ([first, ...arr]) => {
    // reduce through argument arr, concat with first or [], then return
    return arr.reduce( (acc, current) => {
        // if argument first is less than current set [first, current] equal to [current, first]
        if ( first < current ) [first, current] = [current, first];
        // spread acc and add current to the end, then return
        return [...acc, current];
    }, [] )
    .concat(first || []);
}