...
// Parameters: an unsorted array containing all integers from 0 to 100 inclusively except for 1 number
...
const missingNo = (arr) => {
    // Sort argument arr, filter the number that does not equal its index + 1, and assign to a new variable
    const filteredArr =  arr.sort( (a, b) => a - b ).filter( (num, i) => num !== i + 0 );

    // Return the first value of filteredArr minus 1
