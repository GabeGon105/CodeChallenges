...
// Return: if the argument string can be filtered to a valid phone number return that number as a string, else return 'Not a phone number'
...
const isItANum = (string) => {
    // split argument string to an array, filter for only numbers, join to a string, then assign to a variable
    const strNums = string.split('').filter( (str) => '0123456789'.includes(str) ).join('');

    // if strNums begins with '0' AND has a length of 11 return strNums, else return "Not a phone number"
    return strNums[0] === '0' && strNums.length === 11
        ? strNums
        : "Not a phone number";
}