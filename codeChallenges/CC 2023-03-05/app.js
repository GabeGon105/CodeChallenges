...
// Parameters: 3 arguments, integer age of the driver, string car size, integer rental days
...
const insurance = ( age, car, days ) => {
    // calculate the Cost of the driver's age, and the Cost for the car size, then save to variables
    const ageCost = age < 25 ? 10 : 0;
    const sizeCost = car === "economy" ? 0 : ( car === "medium" ? 10 : 15 );

    // if argument days is less than 0 return 0, else return 50 plus ageCost plus sizeCost times argument days
    return days < 0 ? 0 : ( 50 + ageCost + sizeCost ) * days;
}